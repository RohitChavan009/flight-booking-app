...
// Redux
import { useUpdateFlightMutation } from "@/redux/api";
...
const Update = ({ fields, refetch, handleModalClose }) => {
  ...
  const [updateFlight, { isLoading, isError }] = useUpdateFlightMutation();
  ...
  useEffect(() => {
    if (fields) {
      ...
      setDates({
        departureDateTime: dayjs(departureDateTime),
        arrivalDateTime: dayjs(arrivalDateTime),
      });
    }
  }, [reset, fields]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      updateFlight({ id: fields?.id, ...payload })
        .unwrap()
        .then(() => {
          if (!isError) {
            // Success toast 🎉
