...
const DesktopSideBar = ({ children, menu }) => {
  ...
  useEffect(() => {
    ...
    const route = currentRoute === "" ? "Flights" : currentRoute;
  ...
  // Not rendering sidebar for login & signup page 🚨
  if (
    pathName === "login" ||
  ...
  const listItemClickHandler = (text) => {
    const path =
      text === "Flights" ? "/" : text.toLowerCase().replace(/ /g, "_");
          ...
          <div className="flex flex-col gap-4 justify-center items-center mt-2">
            ...
            >
              ...
              <Typography
                ...
                fontWeight={"500"}
              >
                {user?.role} - {user?.id.slice(0, 5)}
              </Typography>
            </Typography>
          </div>
        )}

        {menu.TOP_MENU.length > 0 && (
          <MenuList
            open={open}
